...
Мы хотим предсказать t, t+1

$\alpha = 0.5$

$\hat{y}_{t+1} = 0.5 \cdot 11 + 0.25 \cdot 12 + 0.125 \cdot 10 = 9.75$

$\hat{y}_{t+2} = 0.5 \cdot 9.75 + 0.25 \cdot 11 + 0.125 \cdot 12 + 0.0625 \cdot 10 = 9.75$

А все потому что 9.75 уже содержит информацию о всех предыдущих значениях

если расписать 9.75 как $\hat{y}_{t+1}$ то будет видно как ряд перевзвешивается

Причина не в том, что ряд «магически перевзвешивается», а в том, что экспоненциальное сглаживание хранит всю информацию о прошлом в одном числе — текущем сглаженном уровне.

_Модель Хольта (Holt Model)_

![alt text](image-102.png)

Уравнение уровня - оценки уровня ряда (в конкретной точке) 

Уравнение тренда - оценка уровня тренда (в конкретной точке)

параметр $\beta$ влияет на оценку тренда так же как и $\alpha$

![alt text](image-103.png)

параметр $\phi$ это параметр затухания тренда 

![alt text](image-104.png)

При $\phi$ = 0.95 затухание совсем небольшое
...
С мультипликативной сезонностью работают редко, потому что проще логарифмировать ряд и работать с аддитивностью ведь $log(ab) = log(a) + log(b)$ где a - условно сезонность, b - условной тренд

В случае с аддитивной сезонностью, она просто прибавляется, а в случе с мультипликатовной - умножается

Мультипликативная сезонность завязана на тренд, поэтому если мы выбираем модель без тренда, то мультипликативная сезонность будет бесполезна

![alt text](image-108.png)

Все модели семейства экспоненциального сглаживания

VAR - коинтеграция 

Коинтеграция != влияние экзогенных признаков

_ETS(Error-Trend-Season)_

Мы просто добавляем к нашему семейству моделей экспоненциального сглаживания в уровнение ошибку, которая распределена нормально (белый шум)

Тепреь, когда мы добавили эту ошибку, наша модель стала вероятностной и мы можем строить прогнозные интервалы и оценивать нашу модель методом максимального правдоподобия
